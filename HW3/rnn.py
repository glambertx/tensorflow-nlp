...
class Config(object):
    ...
    # 2nd lr = 0.01 l2 = 0.02
    # 3rd lr = 0.01 l2=0.01
    # 4th lr = 0.008 l2 = 0.01
    #5th lr = 0.02 l2 = 0.02
    #6th lr = 0.01 l2 = 0.02 no numerical stabilizing
    # Training acc (only root node): 1.0
    # Valiation acc (only root node): 0.79, highest 0.81
    #7th lr =0.01 l2 = 0.03 final and highest validation 0.82
    #8th lr = 0.01 l2 = 0.05
    #9 base with dropout
    #10 l2 = 0.03 with dropout final model
...
class RNN_Model():
    ...
    def add_model_vars(self):
        ...
        self.regularizers =  (tf.nn.l2_loss(W1) + tf.nn.l2_loss(U))
            ### END YOUR CODE
    ...
    def add_model(self, node):
        ...
        with tf.variable_scope('Composition', reuse=True):
            ### YOUR CODE HERE
            embedding = tf.get_variable("embedding",[len(self.vocab),self.config.embed_size])
            W11 = tf.get_variable("W1",[2*self.config.embed_size,self.config.embed_size])
            b11 = tf.get_variable("b1",[1, self.config.embed_size])
        ...
        if node.isLeaf:
            ...
            w= tf.concat(1, [node_tensors[node.left], node_tensors[node.right]])
            curr_node_tensor =  tf.nn.relu(tf.matmul(w, W11) + b11)
    ...
    def add_projections(self, node_tensors):
        ...
        with tf.variable_scope('Projection', reuse=True):
            ...
            w2 = tf.get_variable("U",[self.config.embed_size,self.config.label_size])
            b2 = tf.get_variable("bs",[1,self.config.label_size])

            logits = tf.matmul(node_tensors,w2)+b2
            logits = tf.nn.dropout(logits, 0.8)

        """
        ### get max and reshape
        node_max = tf.reduce_max(outputs,reduction_indices=1)

        node_max.np = np.asarray(outputs.eval())
        node_max.np_rows = len(node_max.np[:,])

        node_max = tf.reshape(node_max, shape=[node_max.np_rows,1])

        #remove max from logits
        outputs -= node_max
        logits = outputs
        """
        ### END YOUR CODE
    ...
    def loss(self, logits, labels):
        ...
        loss +=  self.config.l2 * self.regularizers
        # END YOUR CODE
        return loss
