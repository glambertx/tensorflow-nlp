...
def softmax(x):
  ...
  with sess.as_default():
      ...
      ### get max and reshape
      xmax = tf.reduce_max(x,reduction_indices=1)

      xmax.np = np.asarray(x.eval())
      xmax.np_rows = len(xmax.np[:,])

      xmax = tf.reshape(xmax, shape=[xmax.np_rows,1])

      ### Softmax
      x = x - xmax
      exp = tf.exp(x)
      denominator = tf.reduce_sum(exp,1,keep_dims=True)
      out = exp/denominator
