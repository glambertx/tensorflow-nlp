...
class NERModel(LanguageModel):
  ...
  def add_model(self, window):
    ...
    Hint: Here are the dimensions of the various variables you will need to
          ...
          window: tf.Tensor of shape (-1, window_size*embed_size)
          b1: (hidden_size,)
          U:  (hidden_size, label_size) (100x5)
          b2: (label_size)
    ...
    with tf.variable_scope("Layer"):
        W = tf.get_variable("Words",(150,100),initializer=xavier_weight_init())
        U = tf.get_variable("U1",(self.config.hidden_size,self.config.label_size),initializer=xavier_weight_init())

    with tf.variable_scope("Softmax"):
        b1 = tf.get_variable("b1",(self.config.hidden_size,))
        b2 = tf.get_variable("b2", (self.config.label_size))
        ## softmax
        drop_W = tf.nn.dropout(W, self.config.dropout)
        z2 = tf.matmul(window, drop_W) + b1
        a2 = tf.tanh(z2)
        self.drop_a2 = tf.nn.dropout(a2, self.config.dropout)
        self.z3 = tf.matmul(self.drop_a2,U) + b2

    # store parameters
    self.regularizers =  (tf.nn.l2_loss(W) + tf.nn.l2_loss(b1) + tf.nn.l2_loss(b2) + tf.nn.l2_loss(U))
    output = tf.nn.softmax(self.z3)
    self.output = output
    self.U = U
  ...
  def add_loss_op(self, y):
    ...
    """
    ### YOUR CODE HERE
    #losses = -tf.reduce_sum(y*tf.log(tf.nn.softmax(self.z3)))
    #______________________________
    losses = tf.nn.weighted_cross_entropy_with_logits(self.z3,y,0.0001)
    losses +=  self.config.l2 * self.regularizers
